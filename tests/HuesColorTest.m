...
- (void)testLowerCasePreference
{
  ...
  XCTAssertFalse(useLowercase, @"Not lowercase by default");
  XCTAssertEqualObjects([[NSColor whiteColor] hues_hex], @"#FFFFFF", @"default should be uppercase");
  
  [HuesPreferences setUseLowercase:YES];
  useLowercase = [HuesPreferences useLowercase];
  
  XCTAssertTrue(useLowercase, @"Not lowercase by default");
  XCTAssertEqualObjects([[NSColor whiteColor] hues_hex], @"#ffffff", @"default should be uppercase");
}

...
- (void)drawRect:(NSRect)dirtyRect
{
  ...
  NSBezierPath *loupe = [NSBezierPath bezierPathWithRoundedRect:NSInsetRect([self bounds], 1.5, 1.5) xRadius:(LOUPE_SIZE / 2) yRadius:(LOUPE_SIZE / 2)];

  CGImageRef screenShot = CGWindowListCreateImage(rect, kCGWindowListOptionOnScreenBelowWindow, (unsigned int)[window windowNumber], kCGWindowImageDefault);
  ...
  CGContextDrawImage(ctx, [self bounds], screenShot);
  ...
  [loupe setLineWidth:3];
  [loupe stroke];
  
  
  NSLog(@"color: %@", [[self colorOfImage:screenShot atPixel:NSMakePoint(NSMidX(b), NSMidY(b))] hues_hex]);
